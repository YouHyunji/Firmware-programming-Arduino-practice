...
int pinLED_Switch = 11; // 투명 LED (스위치)
...
void loop()
{
   ...
   val_Z_3 = digitalRead(Digital_Z);   // 스위치 : 디지털
  ...
  // 스위치 눌렀을 때 LED
    if(val_Z_3 == LOW) {   // 0일때 ( 눌렀을 때 :PULLUP )
    Serial.println("Switch = HIGH");
    digitalWrite(pinLED_Switch, HIGH);    // 스위치 클릭 LED 켜짐 (흰색)
  }
  else {
    digitalWrite(pinLED_Switch, LOW);
  }
